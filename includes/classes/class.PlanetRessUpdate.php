...
if ( $this->USER['technocratic_time'] <= $hoytime AND $this->USER['technocratic'] == 1) {
	...
	public function SetNextQueueElementOnTop()
	{
		...
		while ($Loop == true)
		{
			...
			$HaveRessources 	 = IsElementBuyable ($this->USER, $this->PLANET, $Element, true, $ForDestroy, ($this->USER['geologe'] >= 1));
	...
	public function SetNextQueueTechOnTop()
	{
		...
		while ($Loop == true)
		{
			...
			$HaveRessources 	= IsElementBuyable($this->USER, $this->PLANET, $Element, true, false, $this->USER['geologe'] >= 1);
