...
class ShowResearchPage
{
	...
	public function AddBuildingToQueue($Element, $AddMode = true)
	{
		...
		if($ActualCount == 0)
		{	
			if(!IsElementBuyable($USER, $PLANET, $Element, true, false, ($USER['geologe'] >= 1)))
				return;
	...
	public function __construct()
	{
		...
		foreach($reslist['tech'] as $ID => $Element)
		{
			...
			$CanBeDone               = IsElementBuyable($USER, $PLANET, $Element, true, false, ($USER['geologe'] >= 1));
		...
		$skin_raza = "voltra";
			...
			$ResearchList[] = array(
				...
				'link'  	=> $TechnoLink,
				'irv' => $irv,
				'oldlink'  	=> false,//MAX_RESEACH_QUEUE_SIZE == 1,
				'queue'  	=> $TechQueue,
			);
		}
