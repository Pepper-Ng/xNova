...
{	
	global $CONF;
	// if ( NO_NOOBPROTECT || NO_NOOBLIMIT || OWNER_NOOB && TARGET_NOTSTRONG || TARGET_BANNED || TARGET_INACTIVE
	if($CONF['noobprotection'] == 0 || $CONF['noobprotectionmulti'] == 0 || ($OwnerPlayer['total_points'] <= $CONF['noobprotectiontime'] && ($OwnerPlayer['total_points'] * $CONF['noobprotectionmulti'] < $TargetPlayer['total_points'])) || $Player['banaday'] > TIMESTAMP || $Player['onlinetime'] < (TIMESTAMP - 60 * 60 * 24 * 7))
	...
	return array(
		...
		// NoobPlayer if: TargetPoints < Noob && USERPOINTS > Noob	editbypepper
		'NoobPlayer' => (($TargetPlayer['total_points'] < $CONF['noobprotectiontime'] && $OwnerPlayer['total_points'] >= $CONF['noobprotectiontime']) || $OwnerPlayer['total_points'] > $TargetPlayer['total_points'] * $CONF['noobprotectionmulti']),
...
function CheckNoobProtec($OwnerPlayer, $TargetPlayer, $Player)
{
	...
	if(
		...
		|| $Player['onlinetime'] < (TIMESTAMP - 60 * 60 * 24 * 7)
	) {
		return array('NoobPlayer' => false, 'StrongPlayer' => false);
	}
	...
	return array(
		'NoobPlayer' => (
			/*
				When Player has less than 5000 (default) points OR
				When TargetPlayer has less than 20% of the Players points
			*/
			($TargetPlayer['total_points'] <= $CONF['noobprotectiontime']) || // Default: 5.000
			($OwnerPlayer['total_points'] > $TargetPlayer['total_points'] * $CONF['noobprotectionmulti'])
		), 
		'StrongPlayer' => (
			/* 
				When Player has less than 5000 points AND
				When TargetPlayer is more than 20% stronger
			*/
